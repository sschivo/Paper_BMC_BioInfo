...
\title{Formal models of biological networks in \animo2{}} %Building formal models of biological networks avoiding formal training}
...
must deal makes it nearly impossible to reason on
...
The software tool ANIMO, recently updated to version 2, allows the biologist to develop
a formal model through a user friendly interface,
...
% \subsection*{Reaction-centered VS reactant-centered}\label{sec:animo-comparison}
% We will now apply some basic model checking queries to the case study presented in~\cite{animo-bibe},
...
% UPPAAL is used in ANIMO to obtain simulation runs from a \tas\ model. While that process is
% very fast, 
...
% Next, we set out to test the model checking performances using the old \tas\ model used in ANIMO and
% the new one introduced in \animo2{}.
...
%(i.e., deadlock due to all automata being in the {\sf not\_reacting} location).
...
% , while probabilistic queries can still be answered within a minute.
% If the choice for TNF$\alpha$ is made instead more restricted (for example, in the interval $\{5, \dots, 10\}$),
% it is still possible to answer exact model checking queries (the property asking
% whether a state in which MK2 is active for more than 40/100 within 30 minutes is found true in 0.6 seconds). The corresponding statistical
% model checking query can be solved faster, but at the expense of approximation (the probability that
% MK2 gets activated to more than 40/100 before 30 minutes is believed to be inside the interval $[0.87, 0.97]$
% with a confidence of $0.95$, and the result was obtained within 8 seconds performing 111 simulation runs).
...
% If we lower the maximum amount of TNF$\alpha$ to 10, however, we obtain that 
% Obtaining an answer to the corresponding exact query (``is it possible to reach a state in which MK2 is active
% before 40 minutes?'') would take too long even if using the new model introduced in \animo2{}.
...
% % The reaction-centered ANIMO model from~\cite{animo-bibe} would generate three
...
\begin{figure}[htbp]
\begin{center}
   \includegraphics[width=.48\textwidth]{images/screenshotMAPKmodel}
\end{center}
...
{\bf \protect\subref{fig:large-model-graph3}}~Modeled activation of JNK1 and MK2 after stimulation with 5 ng/ml TNF$\alpha$ + 10 $\mu$g/ml C225 (2 hours).
\\
...
\def\reactantTA{\includegraphics[scale=0.4]{images/Reactant_R3}}
\newlength\reactantTAheight
\setlength\reactantTAheight{\heightof{\reactantTA}}

\begin{figure}[htb]
  \begin{center}
  \subfloat[\label{fig:network-abcd}]{\begin{minipage}[c][\reactantTAheight]{0.2\textwidth}\begin{center}\includegraphics[scale=1.0]{images/network_abcd}\end{center}\end{minipage}} \qquad\qquad
...
in~{\protect\subref{fig:ta-new-model}} have the same content as the corresponding guards and updates in ({\bf\protect\subref*{fig:ta-old-BA}\--{}\protect\subref*{fig:ta-old-DA}}),
...
\begin{figure}[!htb]
\begin{center}
  \subfloat[Reaction-centered]{\includegraphics[scale=0.07]{images/grafico_ngf_reaction_10levels}\label{fig:comparison-graph-animo-reaction}}
...
\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{images/model_checking_ui}
  \end{center}
