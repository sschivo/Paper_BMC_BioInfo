...
\title{Formal models of biological networks in ANIMO 2} %Building formal models of biological networks avoiding formal training}
...
achieving useful insight without the need to study any new mathematical formalisms.
\end{abstract}
...
{\Large [[briefly describe ANIMO, its modelling assumptions and its intended use]]}
...
In addition to some new features, the new ANIMO 2.0 provides a boost in terms of performances
...
performance improvement is even more significant in this case, increasing
the potential for interactivity also in this area.
...
of the network during a given period of time. As this type of analysis is also normally
...
Our first experiment compares the execution time for simulation runs on the \tas\ models
...
% the new one introduced in ANIMO 2.
...
When a model becomes particularly complex, with many events occurring concurrently,
the state space of the model tends to \emph{grow exponentially} in size, dramatically
increasing the time requested for the analysis of bigger models. To avoid the so-called
state space explosion, an approach that is becoming popular: the so-called \emph{statistical
model checking}.
...
GINsim~\cite{ginsim} & Boolean Networks & No & Yes & Yes & Yes & Yes~$^2$
      \\[5mm]
% \noalign{\vskip 2mm} 
GNA~\cite{gna}    & ODE & No & Yes & Yes & Yes & No~$^3$
      \\
% \noalign{\vskip 2mm} 
Rhapsody~$^4$
